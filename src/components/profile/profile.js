...
const { 
  ...
  reset : resetProfile,
  formState: { isValid: isValidProfile, isDirty: isDirtyProfile ,errors:errorsProfile } 
...
const { 
  ...
  watch: watchPass,
  reset : resetPass,
  formState: { isValid: isValidPass, isDirty: isDirtyPass ,  errors:errorsPass} 
} = useForm({ mode: "onChange" });
...
const formatMyDate = (dateString) => {
  ...
  async function updatePassword(user_pass){
      dispatch(startSpinner());
      setLoadPass(true);
      const {  error } = await supabase.auth.updateUser({ password: user_pass.password });
      setMessage(true);
      if(error){
        setToatMessageBody(error)
      setTimeout(() => {
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6 ">
          <div className="bg-white dark:bg-dark-neutral-800 p-3 rounded-3xl shadow-sm border border-neutral-100 dark:border-white/5">
            ...
            {isEditing ? (
              ...
              <dialog id="my_modal_4" className="modal w-[100%]">
                  <div className="modal-box w-11/12 max-w-5xl flex flex-col items-center  bg-light-neutral-50 dark:bg-dark-neutral-800">
                      <h1 className="mb-7 text-light-secondary-600 dark:text-dark-secondary-500 text-28 font-bold text-center">Update Your Password</h1>

                        <form method="dialog" onSubmit={handleSubmitPass(updatePassword)} className=" flex flex-col items-center w-[100%] ">
                          {/*Password */}
                        <div className="w-[90%] md:w-[60%] lg:w-[50%]">
                          <label className="block font-bold lg:text-16 text-14 font-opensans text-light-primary-800 ml-3 mb-2
