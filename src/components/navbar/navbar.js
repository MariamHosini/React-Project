...
const toggleMenu = () => setIsOpen(!isOpen);
 ...
 function ChangeTheme(){
  ...
  return (
    <>
      <navbar className="flex items-center  lg:p-2 w-full max-w-full mb-3">
         {/* Mobile menu */}
          <div className="mt-5 flex-[7%] col-span-1 order-3 lg:hidden z-40  relative ">
            <div className="dropdown dropdown-end ">
              <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden"  onClick={()=>toggleMenu()}>
                <svg xmlns="" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" 
                strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /> </svg>
              </div>
              {isOpen&&
              <ul
                tabIndex="-1"
                className={`menu menu-sm dropdown-content mt-10 transition-all duration-300 ease-in-out 
                  ${isOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-5'}
                  bg-light-neutral-50 dark:bg-dark-neutral-800  z-50 w-screen p-5 shadow mobile-menu `}>
                <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 md:text-20 mb-1 "><NavLink to="/" onClick={()=>closeDropdown()}>Home</NavLink></li>
                <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 md:text-20 mb-1"><NavLink to="/products" onClick={()=>closeDropdown()}>Products</NavLink></li>
                <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 md:text-20 mb-1"><NavLink to="/our-brand" onClick={()=>closeDropdown()}>Our story</NavLink></li>
                <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 md:text-20 mb-1"><NavLink to="/contact-us" onClick={()=>closeDropdown()}>Contact us</NavLink></li>
                <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 md:text-20 mb-1 "><NavLink to="/blog" onClick={()=>closeDropdown()}>Blog</NavLink></li>
                {
                  !isAuth?
                ...
                border-[2px] h-[50px] w-[100px] hover:bg-light-secondary-200 hover:dark:bg-dark-secondary-700
                  text-light-primary-400  font-bold font-playfair text-16">Login</button></Link>  
                :
                <>
                 <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16 mb-1 ">
                 <NavLink to={`/profile/${user.id}`} onClick={()=>closeDropdown()}>
                 ...
                 <span className="text-light-primary-600 dark:text-dark-secondary-300 font-bold">{user.userName}</span></NavLink></li>
                  <button 
                      onClick={() => {
                        dispatch(setLogout());
                        closeDropdown();
                      ...
                      <Link to={"/"}><span className="font-bold text-16">Logout <i className="fa-solid fa-arrow-right-from-bracket"></i></span></Link>
                    </button>
                </>
                }
          ...
          <div className="order-1 hidden lg:flex lg:flex-[47%] lg:justify-start  ">
            <ul className="menu menu-horizontal px-1 desktop-menu">
              ...
              <li className="text-light-secondary-600 dark:text-dark-secondary-500 font-opensans text-16"><NavLink to="/our-brand">Our story</NavLink></li>
         ...
         {/* Cart and login */}
          <div className="mt-5 flex-[35%] flex items-center  md:pr-0 lg:pr-4 lg:gap-4 order-2 lg:order-3 lg:flex-[25%] lg:justify-end justify-evenly">
            <div className="w-[40px] h-[40px] md:w-[50px] md:h-[50px] rounded-xl border-light-secondary-50 border-[2px]
            ...
            {
              !isAuth?<Link  to="login" ><button className=" login_btn hidden
            ...
            <div className="hidden lg:flex md:flex w-14 h-14 rounded-full bg-pink-200 items-center justify-center shadow-soft backdrop-blur-md dropdown dropdown-bottom">
              <div className="text-light-primary-600 font-bold text-24" tabIndex={0} role="button"  >
                 {user.userName?.charAt(0).toUpperCase()}
              </div>
               <ul tabIndex="1" className=" mt-4 dropdown-content menu p-2  bg-light-neutral-50
                dark:bg-dark-neutral-800 backdrop-blur-md border border-light-secondary-700 dark:border-white/10
                 rounded-2xl relative z-50 w-32 animate-in fade-in zoom-in duration-200">
                  <li className="mb-1">
                    <button className="!flex items-center gap-3 !px-4 !py-3 !rounded-xl text-slate-700 dark:text-pink-100
                      hover:!bg-pink-500/20 active:!bg-pink-500/30 transition-colors duration-200" onClick={()=>closeDropdown()}>
                      <Link to={`profile/${user.id}`}> <span className="font-semibold text-sm">Profile</span></Link> </button>
                 </li>
                  <li>
                    <button 
                      onClick={() => {
                      ...
                      className="flex items-center gap-3 px-4 py-3 rounded-xl text-rose-500 hover:bg-rose-50
                       dark:hover:bg-rose-900/20 ">
                      <Link to={"/"}><span className="font-bold text-sm">Logout <i className="fa-solid fa-arrow-right-from-bracket"></i></span></Link>
                    </button>
                  </li>
              </ul>
